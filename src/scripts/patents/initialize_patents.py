...
def main(bootstrap: bool = False):
    ...
    # update terms set term=regexp_replace(term, '[0-9a-z]{0,1}\)[ ](.*)', '\1') where term ~* '^[0-9a-z]{0,1}\)[ ]' and not term ~ '(?:\[|\])' and term not like '%-%-%'
    ...
    # update terms set term=regexp_replace(term, '(?:target(?:ed|ing) adc)', 'adc') where term ~* '\y(?:target(?:ed|ing) adc)\y';
    ...
    # update terms set term=regexp_replace(term, '([a-z]+)inhibitor', '\1 inhibitor')  where term ~* '[b-z]+inhibitor\y';
    # update terms set term=regexp_replace(term, '^vitro', 'in-vitro') where term ~* '^vitro .*';
    # update terms set term=regexp_replace(term, ', rat', '') where term ~* ', rat$';
    ...
    # update terms set term=regexp_replace(term, 'activate$', 'activator') where term ~* '\y activate$';
    ...
    # update terms set term=regexp_replace(term, 'inhibit(?:ion|ing)$', 'inhibitor') where term ~* '\y inhibit(?:ion|ing)$';
    # update terms set term=regexp_replace(term, 'agonist(?:ic)? action$', 'agonist') where term ~* 'agonist(?:ic)? action$';
    # update terms set term=regexp_replace(term, 'receptor activat(?:ion|or)$', 'activator') where term ~* 'receptor activat(?:ion|or)$';
    # update terms set term=regexp_replace(term, 'agonism$', 'agonist') where term ~* 'agonism$';
    ...
    # update terms set term=regexp_replace(term, '\s{2,}', ' ', 'g') where term ~* '\s{2,}';
